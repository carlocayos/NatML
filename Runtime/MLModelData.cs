...
namespace NatML {
    ...
    public sealed class MLModelData : ScriptableObject {
        ...
        [Obsolete(@"Deprecated in NatML 1.0.19. Use `MLEdgeModel` constructor instead.", false)]
        public unsafe MLModel Deserialize () => new MLEdgeModel(this);
        #endregion
        ...
        public static async Task<MLModelData> FromHub (string tag, string accessKey = null) {
            if (embeddedData.TryGetValue(tag, out var embed))
                return embed;
            var modelData = await FromCache(tag) ?? await FromNatML(tag, accessKey);
