...
namespace NatML {
    ...
    /// <summary>
    /// ML model capable of making edge predictions on features.
    ...
    public sealed class MLEdgeModel : MLModel {
        ...
        /// </summary>
        /// <param name="modelData">ML edge model data.</param>
        public unsafe MLEdgeModel (MLModelData modelData) {
            // Check platform to prevent hard crashes :)
            var platform = NatMLHub.CurrentPlatform;
            if (modelData.session.format != NatMLHub.FormatForPlatform(platform))
                throw new InvalidOperationException($"Cannot deserialize {modelData.session.format} graph on {platform}");
            // Create options
        ...
        private static Type ManagedType (DataType type) => type switch {
